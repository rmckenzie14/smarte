...
.satellite{-webkit-animation:satellit-anima 25s linear infinite;-moz-animation:satellit-anima 25s linear infinite;animation:satellit-anima 25s linear infinite;transform:rotate(120deg);z-index:5;color:#F6AE2D;font-size:18px;position:absolute;left:10%;bottom:0}
...
.eye-rz{left:1px;}
.cat-nose{position:absolute;border-radius:20px 20px 70px 70px;width:7px;height:7px;background:linear-gradient(#000,#59A799);z-index:500;margin:19px 16px}
...
@-webkit-keyframes sleep-cat {0%{height:7px;top:12px;border-radius:30px} 35%{height:8px;top:12px;border-radius:30px} 56%{height:4px;top13px} 70%{height:3px;top:15px} 71%{height:8px;top:12px} 85%{height:3px;top:12px} 97%{height:0;top:15px}}
...
<div class="hill">
  <a href="index.php" ><img src="Static/img/logo.jpg" alt="" style="height:50px; width:150px; margin: auto;"></a>
</div>
<span class="Not ready yet">Page - Not Found</span>

<span class="page-404">Not ready yet. Under construction.
</span>
...
include_once("nav.php");
  ?>
      ...
      <table class="table" style=" float: left; width: 87%;" >
        <!-- <thead>
          <tr>
        ...
        <?php
            ...
            //$getOrders = mysqli_query($con, "SELECT productquantity, productprice, productid FROM cart  WHERE userid = '$userid'  ");
              ...
              if($getOrders){
                  ...
                  if(mysqli_num_rows($getOrders) > 0){
                      while($rows = mysqli_fetch_assoc($getOrders)){
                        ...
                        //    <td style="text-align: center">'.$rows['productid'].'</td>
                        ...
                        //   echo ' <tr>
                        //   <th scope="row" style="text-align: center">'.$count.'</th>
                        //   <td style="text-align: center"><img src="'.substr($rows['productimg'], 3).'" alt="Product Image" id="logoImg"></td>
                      ...
                      foreach($cartArray as $items){
                        echo ' <tr>
                          <th scope="row" style="text-align: center">'.$count.'</th>
                          <td style="text-align: center"><img src="'.substr($items['productimg'], 3).'" alt="Product Image" id="logoImg"></td>
                          <td style="text-align: center">'.$items['productName'].'</td>
                          <td style="text-align: center">'.$items['productquantity'].'</td>
                          <td style="text-align: center">$'.$items['productprice'].'</td>
                          <td style="text-align: center" >'.$items['productDes'].'</td>
                          <td style="text-align: center" >$'.$items['productprice']*$items['productquantity'].'</td>
                        </tr>';
                       
                        
                        $count++;

                        $totalPrice += $items['productprice']*$items['productquantity']; //$items['productprice'];
                      }

                      echo '<tr>
                      <td style="text-align: center" >Total: '.number_format($totalPrice, 2,'.', '.').'</td>
                      </tr>';
    ...
    <div style="display: inline-block; width: 10%; float: right;">
                <?php
echo "Cart total: USD $".number_format($totalPrice, 2,'.', '.');
                ?>
    </div>
      </div>
</body>
